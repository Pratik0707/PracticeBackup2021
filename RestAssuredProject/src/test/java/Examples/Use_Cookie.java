...
public class Use_Cookie {
	...
	public void add()
	{
		...
		//		String StrRes = 
		given()		
		.auth().preemptive().basic(“username”,”password”)// can use this as well if not sending credentials in body (we are now sending in body in the next line, so this line is not required in this case)
		...
		String sessionId = js.get("value"); // get session id and use in below req. as 
		...
		.header("Content-Type","application/json")
		.header("Cookie",sessionId) // we use the session id we got above
		.body("{\r\n"
				+ "    \"body\": \"My comment 132.\",\r\n"
				+ "    \"visibility\": {\r\n"
				+ "        \"type\": \"role\",\r\n"
